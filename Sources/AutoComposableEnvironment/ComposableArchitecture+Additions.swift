...
public extension Reducer where Environment: AutoComposableEnvironment {
  ...
  /// global state, action and environment when the local environment is a subclass of ``AutoComposableEnvironment``.
  ...
  ///
  /// Because the environment is ``AutoComposableEnvironment``, its lifecycle is automatically managed by the library.
  ...
  /// Transforms a reducer that works on local state, action, and environment into one that works on
  /// global state, action and environmentwhen the local environment is a subclass of ``AutoComposableEnvironment``.
  ///
  /// It accomplishes this by providing 2 transformations to the method:
  ///
