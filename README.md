...
- With the current implementation, there is maybe a slight performance cost when dependencies are accessed (the library have to merge the parent's dependencies with the child ones), but no optimization was realized yet, so I guess there's room for improvement.
